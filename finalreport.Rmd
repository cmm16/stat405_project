...
## Data 
Our project utilized 5 data sets: flight, passenger, sentiment synonym, review, logos. 
...
In order to examine our research questions, we conducted some exploratory analysis of the data. The goal of our EDA was to examine patterns or trends in the data to determine the important characteristics and get a sense for the data beyond our initial glances. We created visualizations using data from our flights, reviews, and passengers datasets. 

#### Texas Airports Top Destinations
For our first graph, we wanted to get an understanding on the main airports in Texas. Since we had already decided to focus our report on IAH, we looked at the three other most traveled airports in Texas as a comparison to IAH. The graph shows the top 20 most common destinations out of IAH, DFW, AUS, and DAL. Although the airports each had their own popular destinations, we noticed common destinations amongst the airports. IAH and DFW both had ORD as their most traveled destination while DAL and AUS most traveled destination were airports in Texas. HOU and DFW are both big hubs and common places to fly through when traveling so it makes sense that their common destinations are outside of Texas.  

```{r, echo=FALSE, message = FALSE, error = FALSE, warning=FALSE, fig.height=4, fig.width=6}
...
These graphs look at the number of flights leaving IAH in addition to the number of passengers being transported. As we expected, the graphs closely resemble each other as we were able to see the peaks and falls in each respective graphs during the same months. The summer months had large spikes in both number of flights and passengers which is most likely due to the fact that people are going on vacation more in the summer months. August has the largest number of planes leaving IAH however the number of passengers flying decreases from July to August. We think this discrepancy could come from airlines overcorrecting for the summer demand too late in the season.   
...
The graph showing the number of empty seats on flights out of IAH is a visualization for the passenger inefficiencies each month. The graph follows similar logic to the graphs associated with IAH flight and passenger data. The summer months have have very few seats that go empty due to the fact that summer is a popular travel time. We see a large spike in September which is an interesting area for further exploration to understand why this pattern exists. The increase in empty seats in August we believe can be attributed to the failure of airlines to predict passengers' inclinations for flying.  
...
We then shifted our analysis to the reviews dataset where we examined the distribution of airport ratings for the 10 most common destinations out of IAH. We used the overall ratings given in the dataset to compare the airports and their accompanying review distributions. Our main take-away was that the review distribution skewed heavily towards the negative side. This makes sense since people are more inclined to leave a review if they had an extreme feeling towards the airport, especially if it was negative. 
...
In order to conduct our sentiment analysis that is discussed more later in the report, we found the 20,000 most common words in all the reviews. From there, we found the 25 most common words in the 20,000 original words and all their synonyms. The graph shows these 25 words and the frequency at which they occurred in our sentiment dataset. We were able to use these words to condense our reviews to aid in the sentiment interpretation of each review.
...
We used shiny to make the plot interactive in our presentation, so you can select a single month and see the results for the selected month. As you select different months, you can see the change over time. For the purposes of the report, we only displayed January data.
...
plot_luggage <- function() {
  ...
  grid.text("Top 10 Airport Destinations from IAH January 2018 Analysis", x = unit(0.50, "npc"), 
...
The next killer plot is a pictorial representation of the market share of airlines commuting passengers from IAH. There are a total of 20 seats in the airplane, where each seat represents approximately 5% of the market share. As depicted, United Airlines had the greatest market share (50%), followed by Mesa Airlines (15%), Express Jet Airlines (10%), Spirit Airlines (10%), American Airlines (5%), Republic Airlines (5%), and lastly all other airlines (5%).

## Conclusion 

### Research Questions

Upon our analysis of IAH (Bush International Airport - Houston, TX) we were able to answer our research questions:

##### What are the most popular airlines and destinations from IAH?

Based on our market share analysis we were able to identify the most popular airlines as United Airlines, Mesa Airlines, Express Jet Airlines, Spirit Airlines, American Airlines, and Republic Airlines. The top destinations (airport) were identified as the following: ATL, CLT, DEN, DFW, DTW, EWR, LGA, MSP, ORD, SFO.

##### How have flight patterns out of IAH changed over time?

Through modelling our flight data we were able to identify yearly trends of departing flights remaining consistent from 2015-2017 with a significant jump in 2018. We also identified the seasonal trend of consistently more flights being taken in the summer months (Jun-Aug).

##### How does airport quality affect the number of flights and passengers going to an airport?

Through our sentiment analysis we identified that the overall rating of an airport doesn’t impact the number of flights and passengers going to that airport. This analysis shows that people’s location and travel preferences weigh considerably more than the quality of the airport.

### Summary

In conclusion, we were able to provide a comprehensive analysis of IAH. We were able to:

1. Reinforce assumptions on seasonal effects on flight patterns
2. Analyze flight patterns over time through a time series, and create a forecasting model with minimal error
3. Develop 2 killer plots to visualize flight pattern and market share data
