...
select *
from (SELECT 
DISTINCT YEAR, MONTH, DAY_OF_MONTH, DAY_OF_WEEK, TAIL_NUM, ORIGIN, DEST, DEP_TIME
FROM flights);")
flights <- dbFetch(res, -1)
dbClearResult(res)
```
...
We assume number of flights is a function of time.
$y(t) = g(t) + s(t) + h(t) + \epsilon$
$y(t)$ is number of flights per day
$g(t)$ is trend component
$s(t)$ is seasonal component
$h(t)$ is compenet due to holidays
$\epsilon$ is the error term assumed to be normal
```{r}
...
ggplot2::labs(x = "Number of days forecasted out", y = "Number of Flights") +
ggplot2::ggtitle("Forcasted number of flights per day out of IAH vs.actual") +
ggplot2::geom_point(na.rm=TRUE) +
ggplot2::geom_line(ggplot2::aes(y = yhat), color = "#0072B2",
                       na.rm = TRUE) +
    ggplot2::theme(aspect.ratio = 3 / 5)

ggplot2::ggplot(df.p, ggplot2::aes(x = horizon, y = mae)) +
ggplot2::labs(x = "Days out for forecast", y = "Mean absolute error") +
ggplot2::ggtitle("Absolute error for forecast") +
ggplot2::geom_point(na.rm=TRUE) 
...
results <- data.frame("TYPE"= c("Train", "Test"), "MAE" = c(train_mae, test_mae), "MAPE" = c(train_mape, test_mape), "RMSE" = c(train_rmse, test_rmse))
results
                           ...
                           ...) 
{
  ...
  return(dyBase %>% dyAxis("y", label = "Temp (F)"))
}
