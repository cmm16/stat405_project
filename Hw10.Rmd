...
avg_delays$DAY_OF_WEEK <- c("Monday", "Tuesday", "Wednesday", 
                            "Thursday", "Friday", "Saturday", "Sunday")
avg_delays$delayTime <- avg_delays$`avg(DEP_DELAY)`
avg_delays$DAY_OF_WEEK <- fct_relevel(avg_delays$DAY_OF_WEEK, "Monday", "Tuesday", 
                                      "Wednesday", "Thursday", "Friday", 
                                      "Saturday", "Sunday")

usingGg <- ggplot(avg_delays, aes(x = DAY_OF_WEEK, y = delayTime, order(days))) + 
  geom_bar(stat = "identity", fill = "red", width = 0.5, color = "black") + 
  labs(x = "Day of the Week", 
       y = "Average Delay Times (Minutes)", 
       title = "Average Delay Times at DFW Airport by Day of the Week")
```
...
Plotting the original plot using ggplot. (as mentioned in OH, we could not get the plots side-by-side)
